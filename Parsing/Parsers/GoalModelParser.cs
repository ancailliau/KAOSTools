...
namespace UCLouvain.KAOSTools.Parsing.Parsers
{
	...
	public sealed partial class GoalModelParser
	{
		...
		public UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement Parse(string input)
		{
			return DoParseFile(input, null);
		}
		
		// File is used for error reporting.
		public UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement Parse(string input, string file)
		{
		...
		// AttributeDecoratedValue := (AttributeAtomicValue S ':' S AttributeAtomicValue) / (AttributeAtomicValue S '=' S AttributeAtomicValue) / (AttributeAtomicValue S '[' S AttributeValue S ']') / AttributeAtomicValue
		...
		private UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement DoParseFile(string input, string file)
		{
			m_file = file;
		...
		private State DoParseLiteral(State state, List<Result> results, string literal)
		{
			...
			results.Add(new Result(this, j, literal.Length, m_input, default(UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement)));
		...
		private State DoParse(State state, List<Result> results, string nonterminal)
		{
			...
			if (!m_cache.TryGetValue(key, out cache))
			{
				...
				UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement value = hasResult ? results[results.Count - 1].Value : default(UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement);
		...
		private State DoParseRange(State state, List<Result> results, bool inverted, string chars, string ranges, UnicodeCategory[] categories, string label)
		{
			...
			if (matched)
			{
				results.Add(new Result(this, state.Index, 1, m_input, default(UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement)));
		...
		{
			public CacheValue(State state, UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement value, bool hasResult)
			{
				State = state;
				Value = value;
				HasResult = hasResult;
			...
			public UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement Value;
		...
		{
			public Result(GoalModelParser parser, int index, int length, string input, UCLouvain.KAOSTools.Parsing.Parsers.ParsedElement value)
			{
				m_parser = parser;
				m_index = index;
				m_length = length;
