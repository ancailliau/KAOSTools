...
namespace KAOSTools.Parsing
{
    public class ThirdStageBuilder : Builder
    {
        ...
        {
            element.DomPre = fb.BuildFormula (conflict.Specification);
        }

        public void Handle (Operation element, ParsedDomPostAttribute conflict)
        {
            element.DomPost = fb.BuildFormula (conflict.Specification);
        }

        public void Handle (Operation element, ParsedReqPreAttribute reqpre)
        {
            Goal goal;

            if (reqpre.IdOrName is IdentifierExpression | reqpre.IdOrName is NameExpression) {
                if (!Get (reqpre.IdOrName, out goal)) {
                    goal = Create<Goal> (reqpre.IdOrName);
                }
            } else {
                throw new NotSupportedException ();
            }

            element.ReqPre.Add (new ReqOpSpecification (
                goal,
                fb.BuildFormula (reqpre.Specification)
            ));
        }

        public void Handle (Operation element, ParsedReqPostAttribute reqpost)
        {
            Goal goal;

            if (reqpost.IdOrName is IdentifierExpression | reqpost.IdOrName is NameExpression) {
                if (!Get (reqpost.IdOrName, out goal)) {
                    goal = Create<Goal> (reqpost.IdOrName);
                }
            } else {
                throw new NotSupportedException ();
            }

            element.ReqPost.Add (new ReqOpSpecification (
                goal,
                fb.BuildFormula (reqpost.Specification)
            ));
        }

        public void Handle (Operation element, ParsedReqTrigAttribute reqtrig)
        {
            Goal goal;

            if (reqtrig.IdOrName is IdentifierExpression | reqtrig.IdOrName is NameExpression) {
                if (!Get (reqtrig.IdOrName, out goal)) {
                    goal = Create<Goal> (reqtrig.IdOrName);
                }
            } else {
                throw new NotSupportedException ();
            }

            var dynamic2 = fb.BuildFormula (reqtrig.Specification);

            element.ReqTrig.Add (new ReqOpSpecification (
                goal,
                dynamic2
            ));
        }
